...
| 04 | Charge timing and capture correctness | L | `COMPLETED` 2026-08-03 audit closure — `CaptureOnTripStartIT` passes and the default-off local fake exercises authorize → capture/void at runtime; smoke 14/14. The audit fixed explicit provider persistence plus three false-negative/false-positive harness defects. Real Cybersource sandbox certification remains an external release gate |
| 05 | Trip timers and reliability | XL | `COMPLETED` 2026-08-03 reverified — all four clocks, panels, deactivation trigger and prepay flag; runtime smoke 42/42. V032 materialises trips at first confirmed booking |
| 06 | Penalties, dues and compensation | L | `COMPLETED` 2026-08-03 reverified — all five kinds assessed, the 50/50 split enforced by CHECK and property test, three collection paths, dues carried and disputes with a 48-hour window. V033 adds the `penalty` schema; Slice 11 compensation is verified in the shared rewards ledger |
...
| 12 | Real-time location pipeline | L | `COMPLETED` 2026-08-04 — batch-idempotent filtered ingest, loop-safe route progress, dead reckoning and fail-closed confidence; private 500 m approach sessions; five adaptive policies; authenticated WebSocket/SSE with FCM/APNs fallback; partitioned retention and 10-second sweeps. Runtime 8/8, 300-row candidate-query p95 0.397 ms. V039 |
