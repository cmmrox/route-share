...
- [~] Phase G — Admin suite rebuild. **G-1 done (V022):** real audit log (`audit.audit_action` + `AdminAuditService` recording every admin mutation), real user management (list/detail/suspend/activate/status-history on `identity.app_user` — suspension is enforced via `upsertFromToken`), and real admin support + SOS management wired to the Phase E tables (status/reply/resolve with audit + raiser notification). Role granularity via method security on each controller. **G-2 remaining (still on the workflow_item readiness facade):** dashboard aggregates, driver-application/vehicle verification + document review/signed-download, trip/booking ops + location trail, fare policies + commission rules, settlements/payout batches/finance adjustments, notification broadcasts, reports/export, and Keycloak role propagation.
