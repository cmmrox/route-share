...
# Task 03 — App Shell, Navigation, State, and Offline Foundation
...
**Goal:** Implement complete navigation and app lifecycle state.

**Scope:** Splash, onboarding, auth/main stacks, search/results/booking/trip/account/safety/support routes.
...
1. Define typed route map and params for searchId, resultId, bookingId, tripId, paymentIntentId, shareLinkId, pickup/dropoff.
2. Implement startup state machine: splash → app config → onboarding check → token check → auth/me → profile completeness → home.
3. Persist onboarding completion, selected home variant, theme, and safe dev-only environment preference.
4. Implement offline/network banner, query retry/cache rules, app-wide toasts, confirmation dialogs, native back behavior, and deep links for auth callback/share trip.
5. Add tests for route guards, back-stack behavior, expired token routing, offline startup, and protected screen access.
...
- [x] iOS and Android have been checked for this feature area via local scaffold smoke/config gates; full device automation remains later release evidence.
...
```bash
git commit -m "feat(passenger-mobile): complete app shell, navigation, state, and offline foundation"
```

## Completion evidence — 2026-06-14 18:20 +0530

Implemented app shell/navigation/state/offline foundation with typed route contracts, startup route guards, persisted preference defaults/validation, offline-aware query/mutation policy, expanded auth session state, deep-link config, offline banner, and placeholder shell route screens for all Task 03 routes.
...
Unit result: `9` files / `40` tests passed.
