...
function Phone({ children, statusDark = false, statusBg = "transparent", navBg = "transparent", navDark = false, width = PHONE_W, height = PHONE_H, label, note }) {
  const NOTE_TINT = {
    good: { bg: "var(--status-approved-soft)", bd: "var(--status-approved)", fg: "var(--status-approved-ink)", icon: "check" },
    warn: { bg: "var(--status-pending-soft)", bd: "var(--status-pending)", fg: "var(--status-pending-ink)", icon: "alert" },
    bad: { bg: "var(--status-rejected-soft)", bd: "var(--status-rejected)", fg: "var(--status-rejected-ink)", icon: "close" },
    info: { bg: "var(--surface)", bd: "var(--line)", fg: "var(--ink-3)", icon: "help" },
  };
  const nt = note ? (NOTE_TINT[note.kind] || NOTE_TINT.info) : null;
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", gap: 10 }}>
      ...
      {/* Review annotation — lives OUTSIDE the device so it never steals screen height */}
      {note && (
        <div style={{
          boxSizing: "border-box", width, padding: "12px 14px", borderRadius: 16,
          background: nt.bg, border: `1px solid ${nt.bd}`, display: "flex", gap: 10,
          fontFamily: "var(--font)", alignItems: "flex-start",
        }}>
          <Icon name={nt.icon} size={16} color={nt.fg} strokeWidth={2.2}/>
          <div style={{ fontSize: 11.5, color: "var(--ink-2)", lineHeight: 1.5, flex: 1 }}>
            {note.head && <b style={{ color: nt.fg }}>{note.head} </b>}{note.text}
          </div>
        </div>
      )}
    </div>
